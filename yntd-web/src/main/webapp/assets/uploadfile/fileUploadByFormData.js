...
;(function($,layer,window,document){
	...
	Uploader.prototype={
		...
		event:function(){
			...
			/*	if(support.fd.has("upload")){
					support.fd["delete"]("upload");
				}*/
			...
			$("#"+_opt.previewBtn).on("click",function(){
					layer.open({
						...
						area : [ '600px', '480px' ],
						content: $("#"+_opt.previewArea),
						yes:function(){
							if($("#"+_opt.fileUriContainer).val()!=""){
								_this.deleteOldFile(_opt.deleteServer,$("#"+_opt.fileUriContainer).val());
							}
						 _this.upload(_opt);
						}
					})	
